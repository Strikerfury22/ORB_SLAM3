...
do
  ...
  T_START=$SECONDS
  if [ $3 -eq 0 ]; then
    $ORIGINAL_PATH/Examples/Stereo/stereo_hist_latency $N_TOKENS_PIPELINE 27 $ORIGINAL_PATH/Vocabulary/ORBvoc.txt $ORIGINAL_PATH/Examples/Stereo/Malaga.yaml $DATASETS_PATH/$dataset $DATASETS_PATH/$dataset/$dataset.txt data_orbslam > orbslam3_output.log 2>&1
  elif [ $3 -eq 1 ]; then
