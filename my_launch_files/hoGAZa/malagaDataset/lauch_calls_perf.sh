#!/bin/bash
if [ $# -lt 3 ]; then
	echo "You need the name of the dataset. Belongs to this list: (Malaga07, Malaga07_unrectified)"
...
do
  ...
  echo "***********************************************************************"
  echo ""
  res_directory=Results_04_10_2024/${dataset}_${N_TOKENS_PIPELINE}_${extResDir}_$i
  ...
  T_START=$SECONDS
  if [ $3 -eq 0 ]; then
    $ORIGINAL_PATH/Examples/Stereo/stereo_euroc_threads_times $N_TOKENS_PIPELINE 27 $ORIGINAL_PATH/Vocabulary/ORBvoc.txt $ORIGINAL_PATH/Examples/Stereo/Malaga.yaml $DATASETS_PATH/$dataset $DATASETS_PATH/$dataset/$dataset.txt data_orbslam > orbslam3_output.log 2>&1
  elif [ $3 -eq 1 ]; then
