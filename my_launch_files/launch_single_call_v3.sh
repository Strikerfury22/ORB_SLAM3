...
do
  ...
  echo "***********************************************************************"
  echo ""
  res_directory=Results/${dataset}_${N_TOKENS_PIPELINE}_$i
  
  rm -fr $res_directory
  mkdir -p $res_directory
  cd $res_directory
  T_START=$SECONDS
  $ORIGINAL_PATH/Examples/Stereo/stereo_euroc $N_TOKENS_PIPELINE 27 $ORIGINAL_PATH/Vocabulary/ORBvoc.txt $ORIGINAL_PATH/Examples/Stereo/EuRoC.yaml $DATASETS_PATH/EuRoC/$dataset $ORIGINAL_PATH/Examples/Stereo/EuRoC_TimeStamps/$dataset.txt data_orbslam > orbslam3_output.log 2>&1
