    ...
    do
        ...
        while [ ! -f $dest_path/data_orbslam_f.txt ]; do #Continuosly check until correctly generated
            ...
            if [ -z "$N_TOKENS_PIPELINE" ]; then #Not provided
                ...
                $ORIGINAL_PATH/Examples/Stereo/stereo_euroc $N_TOKENS_PIPELINE $ORIGINAL_PATH/Vocabulary/ORBvoc.txt $ORIGINAL_PATH/Examples/Stereo/EuRoC.yaml $DATASETS_PATH/EuRoC/$dataset $ORIGINAL_PATH/Examples/Stereo/EuRoC_TimeStamps/$dataset.txt data_orbslam > orbslam3_output.log 2>&1
            fi
